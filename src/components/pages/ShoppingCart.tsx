...
export const ShoppingCart: React.FC = () => {
        ...
        {field: 'price', headerName: 'Price', flex: 0.3, type: 'number'},
        {
            field: 'actions', type: 'actions', flex: 0.1, getActions: (params) => [
                <GridActionsCellItem label="remove" icon={<Delete></Delete>}
                                     onClick={() => {
                                         idProduct.current = params.id as string;
    ...
    return <Box
        ...
        <ConfirmationDialog isOpened={openedConfirmation}
                            handleAgree={async () => await
                                ordersService.removeShoppingProduct(authUser, idProduct.current)}
                            handleClose={() => setConfirmation(false)}
                            handleDisagree={() => setConfirmation(false)}/>
    </Box>
